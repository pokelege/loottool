...
FileManagerService.prototype._listLoop = function (bot, message, user, files, err, response)
{
    ...
    if (response.paging) {
        if(response.paging.page !== undefined && response.paging.pages !== undefined &&
            ...
            bot.api.files.list({ token: bot.config.bot.app_token, page: response.paging.page + 1 }, this._listLoop.bind(this, bot, message, user, files));
            return;
        }
    }

    this._actualDelete(bot, message, user, files);
};
