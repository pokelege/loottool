...
FileManagerService.prototype._actualDelete = function (bot, message, user, files)
{
    ...
    for (var i = 0; i < files.length; ++i) {
        bot.api.files.delete({ token: bot.config.bot.app_token, file: files[i].id });
    }

    this.forcePostPublic(bot, message, "Nuked " + files.length + " files. Remember, blame " + user.name + " for deleting your precious files. (And memes.)");
};
