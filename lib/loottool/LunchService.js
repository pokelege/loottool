...
LunchService.prototype._add = function*(bot, message, day, location) {
	...
	var dayInt = day ? this._dayz[day.toLowerCase()] : new Date().getUTCDay();
	if (dayInt === undefined) {
		dayInt = new Date().getUTCDay();
		...
		if(yield zone.hasNext())
		{
			...
			var local = new Date().getUTCDay() + ( diff < 0 ? -1 : diff >= 24 ? 1 : 0);
...
LunchService.prototype._list = function*(bot, message){
	...
	for(var i = 0; i < dayKeys.length;++i){
		...
		var lunch = db.collection("lunch").find({team_id: message.team_id, day: i});
...
LunchService.prototype.resolveSlash = function (bot, message) {
	...
	if(message.text.includes("add") || message.text.includes("remove")) {
		var addRegex = /(add)?(remove)?\s([sS]unday)?([mM]onday)?([tT]uesday)?([wW]ednesday)?([Tt]hursday)?([Ff]riday)?([Ss]aturday)?\s*([\s\S]*)/gm;
		...
		if(message.text.includes("remove")){
			coroutine(this._remove.bind(this, bot, message, day, results[10]));
		}
		else coroutine(this._add.bind(this, bot, message, day, results[10]));
