...
GachaService.prototype.getCharactersByUser = function(db, user) {
	return db.collection("user_pulls").find({user_id: user.id})
};
...
GachaService.prototype._addCharacter = function* (bot, message, character) {
    ...
    var exist = yield db.collection("characters").find({name: character}).limit(1);
    if(yield exist.hasNext()){
        bot.replyPrivateDelayed(message, "Already added " + this.printCharacter((yield exist.next())));
    } else{
		var star = (Math.floor(Math.random() * 1000) % 6) + 1;
		var newCharacter = {name:character, stars: star};
        yield db.collection("characters").insertOne(newCharacter);
...
GachaService.prototype._battleSelect = function*(bot, message, user) {
	...
	for(var i = 0; i < 20 && (yield list.hasNext()); ++i){
		...
		for(var j = 0; j < 5 && (yield list.hasNext()); ++j){
			...
			var character = yield list.next();
			actions[j].value = character.name;
			actions[j].text = actions[j].value + this._printStars(character.stars);
		}
