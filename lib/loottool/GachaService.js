...
GachaService.prototype._battle = function* (bot, message, character) {
	...
	var haveCharacter = db.collection("user_pulls").find({user_id: message.user_id, name: character}).limit(1).toArray();
	...
	var battle = yield db.collection("battles").find({team_id: message.team_id}).limit(1).toArray();
	if(battle.length < 1){
		yield db.collection("battles").insertOne({team_id: message.team_id, user_id: message.user_id, user_name: message.user_name, name: haveCharacter.name, stars: haveCharacter.stars});
