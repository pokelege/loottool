...
GachaService.prototype._pullCharacter = function*(bot, message, user, isSlash) {
    // Connection URL
    var url = process.env.MONGOLAB_URI;
    // Use connect method to connect to the Server
    var db = yield MongoClient.connect(url);
	...
	if(list.length < 1){
		if(isSlash) bot.replyPrivateDelayed(message, {text: "Nothing to pull", response_type: "ephemeral"});
		db.close();
		return;
	}
    ...
    if(isSlash) bot.replyPublicDelayed(message, {text: toReturn, response_type: "in_channel"});
...
GachaService.prototype.resolveSlash = function (bot, message) {
    ...
    switch(text[0]){
        ...
        case "pull":
			this.getUserInfo(bot,message, function(user){
            coroutine(this._pullCharacter.bind(this, bot, message, user, true))
...
GachaService.prototype.resolveOutHook = function (bot, message) {
	...
	this.getUserInfo(bot,message, function(user){
    coroutine(this._pullCharacter.bind(this, bot, message, user))
