...
GachaService.prototype._listCharacters = function*(bot, message) {
	...
	if (message.text.includes("pool")) {
		...
		list = db.collection("user_pulls").find({user_id: this.getUserId(message)});
...
GachaService.prototype._pullCharacter = function*(bot, message, isSlash) {
	...
	yield db.collection("user_pulls").insertOne({user_id: this.getUserId(message), name:pulled.name, stars: pulled.stars});
	
    var toReturn = this.getUserName(message) + " pulled " + pulled.name + this._printStars(pulled.stars);
    // Close the connection
    db.close();
